...
import { 
  ...
  ShieldCheck, Terminal, Compass, Share2
} from 'lucide-react';
...
export const ProjectFullPageView: React.FC<ProjectFullPageViewProps> = ({
  ...
  // Handle ESC key & body scroll locks cleanly
  useEffect(() => {
    if (!project) return;

    // Completely lock main window scroll
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed inset-0 z-[250] w-full h-full min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F19] text-slate-900 dark:text-slate-100 overflow-hidden font-sans flex flex-col lg:flex-row"
      >
        ...
        {/* ========================================================================= */}
        {/* MOBILE TOP HEADER BAR (Visible on lg:hidden)                               */}
        {/* ========================================================================= */}
        <header className="lg:hidden sticky top-0 z-30 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 px-4 py-3 flex items-center justify-between shadow-sm">
          <button
            onClick={onClose}
            className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-xl font-bold bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 text-xs cursor-pointer border border-blue-200/60 dark:border-blue-900/60"
          >
          ...
          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 rounded-full transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        {/* ========================================================================= */}
        <main className="flex-grow overflow-y-auto p-4 sm:p-8 lg:p-12 space-y-8 bg-[#F8FAFC] dark:bg-[#0B0F19]">
          <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none space-y-6">
              ...
              <div className="p-6 sm:p-8 space-y-3 border-t border-slate-200/60 dark:border-slate-800/60">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3.5 py-1.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-mono font-bold text-slate-800 dark:text-slate-200 shadow-sm"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            {activeTab === 'overview' && (
              ...
              >
                <div className="space-y-3">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">Project Summary & Architecture</h3>
        ...
        {/* Mobile Fixed Footer */}
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 p-3 flex items-center justify-between shadow-lg">
          <button
            onClick={onClose}
