...
export const ActivitySection: React.FC = () => {
  ...
  return (
    <section id="activities" className="relative py-10 sm:py-12 bg-[#F5F7FB] dark:bg-[#0F172A] text-[#0F172A] dark:text-white border-t border-b border-[#E2E8F0] dark:border-slate-800 overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 relative z-10">
        ...
        <div className="space-y-4">
          {engagementActivities.slice(0, 4).map((activity, index) => (
            <motion.div
              ...
              className="group p-4 sm:p-5 rounded-xl bg-white dark:bg-slate-900/80 backdrop-blur-md border border-[#E2E8F0] dark:border-slate-800 flex flex-col md:flex-row items-start md:items-stretch gap-4 hover:border-[#2563EB]/50 dark:hover:border-blue-500/50 transition-all duration-300 shadow-sm cursor-pointer hover:-translate-y-0.5"
            >
              {/* Left content: text details */}
              <div className="flex-grow space-y-3 flex flex-col justify-between w-full">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="flex items-center space-x-1.5 px-2.5 py-0.5 rounded bg-[#EFF6FF] dark:bg-blue-900/30 text-[#2563EB] dark:text-blue-400 font-bold border border-[#DBEAFE] dark:border-blue-800/50 text-[10px] font-mono uppercase tracking-wider">
                      {getCategoryIcon(activity.category)}
                      <span>{activity.category}</span>
                    </span>
                    <span className="text-[11px] text-[#64748B] dark:text-slate-400 font-mono flex items-center bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-100 dark:border-slate-700">
                      <Calendar className="h-3 w-3 mr-1 text-[#2563EB] dark:text-blue-400" />
                  ...
                  <h4 className="text-[#0F172A] dark:text-white font-extrabold text-base sm:text-lg leading-snug tracking-tight group-hover:text-[#2563EB] dark:group-hover:text-blue-400 transition-colors">
                    {activity.title}
                  </h4>
                  <p className="text-[#64748B] dark:text-slate-400 text-xs sm:text-sm leading-relaxed font-normal line-clamp-2">
                    {activity.description}
                  </p>
                </div>
                
                <div className="flex items-center text-[#2563EB] dark:text-blue-400 text-xs font-bold font-mono tracking-wider pt-1 mt-auto">
              ...
              {activity.image?.url && (
                <div className="w-full md:w-48 h-32 md:h-full min-h-[120px] rounded-xl overflow-hidden shrink-0 border border-[#E2E8F0] dark:border-slate-800 bg-slate-100 dark:bg-slate-800/50 relative">
                  <img
        ...
        <div className="flex justify-center pt-6 sm:pt-8">
          ...
          >
            <span>Browse All Engagements</span>
