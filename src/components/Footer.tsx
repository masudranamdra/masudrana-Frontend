...
export const Footer: React.FC = () => {
  ...
  const socials = [
    ...
    {
      name: 'Twitter',
      url: 'https://twitter.com/masudranamdra',
      icon: (
        <svg className="h-4.5 w-4.5 fill-current" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-8 p-4 sm:p-5 rounded-xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="space-y-1 text-center md:text-left">
            <h2 className="text-base sm:text-lg font-extrabold text-white flex items-center justify-center md:justify-start gap-2">
              <span role="img" aria-label="email">📧</span> <span>Let's Build Something Exceptional</span>
            </h2>
            <p className="text-xs sm:text-sm text-slate-400 font-normal">
              Always open to software engineering proposals, contract development, and technical collaborations with <strong className="text-slate-300">Masud Rana (masuddev01, masudranamdra)</strong>.
            </p>
          </div>
          <Link
            href="/#contact"
            ...
            aria-label="Start a conversation with Masud Rana"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-8">
          ...
          <div className="md:col-span-4 space-y-5">
            <Link href="/" className="flex items-center space-x-2 text-white font-extrabold text-2xl tracking-wider" aria-label="Masud Rana Homepage">
              <span className="bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">
                Masud Rana
              </span>
            </Link>
            <p className="text-slate-400 text-sm leading-relaxed font-light">
              {config?.siteDescription || 'Official portfolio of Masud Rana (masudranamdra, masuddev01) — Senior Full Stack Software Engineer crafting web applications, SaaS products, and custom software systems.'}
            </p>
          </div>

          {/* Navigation Links */}
          <nav className="md:col-span-4 space-y-5" aria-label="Footer Navigation">
            ...
            <ul className="grid grid-cols-2 gap-y-3 text-slate-400 text-sm font-light">
              ...
              <li>
                <Link href="/gallery/images" className="hover:text-white hover:underline transition-all">Media Vault</Link>
              </li>
              <li>
          ...
          <div className="md:col-span-4 space-y-5">
            ...
            <div className="space-y-3.5 text-slate-400 text-sm font-light">
              <div className="flex items-center space-x-3 group">
                ...
                <a href={`mailto:${config?.contactEmail || 'masudranamdra@gmail.com'}`} className="hover:text-white transition-colors" aria-label="Send email to Masud Rana">
                  {config?.contactEmail || 'masudranamdra@gmail.com'}
                </a>
              </div>

              <div className="flex items-center space-x-3 group">
                <MessageSquare className="h-4.5 w-4.5 text-emerald-400 shrink-0 group-hover:scale-110 transition-transform" />
                <a href="https://wa.me/8801877080660" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors" aria-label="Chat on WhatsApp with Masud Rana">
            ...
            {/* Social Icons with rel="me" attributes */}
            <div className="flex items-center space-x-3.5 pt-3">
              {socials.map((soc) => (
                <a
                  ...
                  aria-label={`Masud Rana on ${soc.name} (${soc.name.toLowerCase() === 'twitter' ? 'masudranamdra' : 'masuddev01'})`}
        ...
        <div className="border-t border-[#1E293B] pt-8 flex flex-col md:flex-row justify-between items-center text-slate-500 text-[10px] sm:text-xs">
          <p>© {currentYear} Masud Rana (masuddev01 / masudranamdra). All rights reserved.</p>
