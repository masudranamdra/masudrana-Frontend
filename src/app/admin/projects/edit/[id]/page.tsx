...
import {
  ...
  FolderCode, Globe, CheckCircle2, AlertCircle
} from 'lucide-react';
...
export default function EditProjectAdminPage() {
      ...
      try {
        ...
        if (res.data?.success && res.data.data) {
          ...
          setFormFields({
            ...
            demoLink: p.demoLink || '',
            image: p.image?.url || '',
            gallery: p.gallery || (p.documentDetails?.images ? p.documentDetails.images.map((url: string) => ({ url, publicId: '', caption: '' })) : []),
            isFeatured: p.isFeatured ?? true,
            featuredInAbout: p.featuredInAbout ?? false,
            order: p.order || 0,
          });
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fileObject) {
        ...
        res = await API.put(`/projects/${projectId}`, payload);
      }
      ...
      triggerToast(err.response?.data?.message || 'Error updating project', 'error');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <form id="editProjectForm" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
